...
console.log(data, 'ha data');
  ...
  const downLoadPdf = async () => {
    ...
    const pageWidth = pdf.internal.pageSize.getWidth() - leftMargin - rightMargin;
    ...
    const titleWidth = pdf.getStringUnitWidth(formTitle) * styles.formTitle.fontSize / pdf.internal.scaleFactor;
    ...
    pdf.text(`Submitted: ${new Date().toLocaleDateString()}`, pageWidth - rightMargin - 50, yPosition);
    yPosition += 10;
    ...
    const formatAddressAnswer = (address, x, y) => {
      const lineHeight = 5;
      const padding = 5;
      const addressParts = address ? Object.entries(address)
        .filter(([_, v]) => v)
    ...
    for (const field of data.response_data || []) {
      const questionText = field.question_title || 'Untitled Question';
      let answerText = '';
      let isLink = false;
      ...
      let questionHeight = addFormattedText(questionText, 0, 0, questionColWidth, styles.question, false);
      let answerHeight = 0;
  
      if (isAddress) {
        answerHeight = formatAddressAnswer(field.response, leftMargin + questionColWidth, yPosition);
      } else {
        switch (field.field_type) {
          ...
          case 'file':
            ...
            break;
          case 'checkbox':
            answerText = Array.isArray(field.response) && field.response.length > 0
              ? field.response.join(', ')
              : 'No options selected';
            break;
          case 'dropdown':
          ...
          case 'phone':
            ...
            break;
          case 'date':
            answerText = field.response ? new Date(field.response).toLocaleDateString() : 'No date selected';
            break;
          default:
            answerText = field.response || 'No response';
        }
  
        answerHeight = addFormattedText(answerText, leftMargin + questionColWidth, yPosition, answerColWidth, styles.answer, isLink);
      }
      ...
      pdf.setFillColor(...styles.answer.fillColor);
      pdf.rect(leftMargin + questionColWidth, yPosition, answerColWidth, finalRowHeight, 'F');
      pdf.rect(leftMargin + questionColWidth, yPosition, answerColWidth, finalRowHeight);
  
      addFormattedText(questionText, leftMargin, yPosition, questionColWidth, styles.question, false);
  
      if (isAddress) {
        formatAddressAnswer(field.response, leftMargin + questionColWidth, yPosition);
      } else {
        addFormattedText(answerText, leftMargin + questionColWidth, yPosition, answerColWidth, styles.answer, isLink);
      }
  
      yPosition += finalRowHeight + 3;
    }
  
    pdf.save(`response_${data.form_title, ' ', data.responder_email}  ${Date.now()}.pdf`);
  };
